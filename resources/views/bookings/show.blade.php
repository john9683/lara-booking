<x-app-layout>
    ...
    <div class="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        <div class="flex justify-start item-start space-y-2 flex-col">
            <h1 class="text-3xl lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">Отель {{ $booking->hotelTitle }}</h1>
        </div>
        <div class="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
            <div class="bg-gray-50 w-full xl:w-96 flex justify-between items-center md:items-start px-4 py-6 md:p-6 xl:p-8 flex-col">
                    ...
                    <div class="w-full flex flex-col justify-start items-start flex-shrink-0">
                        <div class="flex justify-center w-full md:justify-start items-center space-x-4 py-4 border-b border-gray-200">
                            ...
                            <div class="flex justify-start items-start flex-col space-y-2">
                                <p class="text-base font-semibold leading-4 text-left text-gray-800">{{ $booking->userName }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-center text-gray-800 md:justify-start items-center space-x-4 py-4 w-full">
                            ...
                            <p class="cursor-pointer text-sm leading-5 ">{{ $booking->userEmail }}</p>
                        </div>

                        @if($booking->cancelDate)
                            <p class="cursor-pointer text-sm leading-5 ">Дата отмены: {{ $booking->cancelDate }}</p>
                            <p class="cursor-pointer text-sm leading-5 ">Причина отмены: {{ $booking->cancelReason }}</p>
                            <p class="cursor-pointer text-sm leading-5 ">
                                @if($booking->cancelUserSelf)
                ...
                <x-bookings.booking-card :booking="$booking" :cancelBooking="$cancelBooking" :cancelReasonArray="$cancelReasonArray"></x-bookings.booking-card>
        </div>
    </div>
</x-app-layout>
